...
void MainWindow::OpenPrinter()
{
    if (!pAutoPrinter)
    {
        ...
        long nTimeout = 5000;
        int x = pPrinterInstance->Print_ExtraCommand(nTimeout,"Set Printer Model",(LPVOID)strPrinterModel.c_str(),reply,ec);
        if (x != 0)
        {
            OutputMsg("Failed in Set Printer mode:%d",x);
            delete pAutoPrinter;
            pAutoPrinter = nullptr;
            return;
        }
        x = pPrinterInstance->Print_Open((char *)strPort.c_str(),(char *)strAddress.c_str(),ec);
        if (x != 0)
        {
        ...
        else
        {
           ...
           r = pPrinterInstance->Print_ExtraCommand(nTimeout, (char*) "setCardSideEject",(char*) "FRONT_SIDE_UP", reply, ec);
           OutputMsg("> setCardSideEject: %d\n", r);
...
void MainWindow::Printer_Start()
{
    ...
    pPrinterInstance->Print_ExtraCommand(lTimeout,"Set SavePreview",(LPVOID)"/mnt/internal_sd/wltlib/PrintPriew.bmp",lpCmdOut,szRCode);
...
////    {
      string strFont = "宋体";
      pPrinterInstance->Print_PrintText(lTimeout,"姓名  测试用户",nAngle,28,14.5,"SIMSUN.ttf",size,1,0,szRCode);
      pPrinterInstance->Print_PrintText(lTimeout,"社会保障号码  123456789012345678",nAngle,28,19,"SIMSUN.ttf",size,1,0,szRCode);
      pPrinterInstance->Print_PrintText(lTimeout,"社会保障卡号  ABCDEFGHIJKLMN",nAngle,28,23.5,"SIMSUN.ttf",size,1,0,szRCode);
      pPrinterInstance->Print_PrintText(lTimeout,"发卡日期  2019年9月27日",nAngle,28,28,"SIMSUN.ttf",size,1,0,szRCode);
//        if (pPrinterInstance->Print_PrintText(lTimeout,(char *)imageText.toStdString().c_str(),nAngle,x,y,"宋体",size,1,0,szRCode))
