...
#define MWERROR_LOADLIBRARY          -0X0042  //加载动态库失败
#define MWERROR_GETIMAGE             -0X0043  //获取身份证图片信息失败
#define MWERROR_EXCEPTIONS           -0X0044  //出现异常
#define MWERROR_ILLEGAL_CARD         -0X0045  //非法卡
...
#define  IFD_M1READERR  -200008  //读块数据错误
...
extern "C"{
    ...
    */
    ...
    /*
    * Function:	mwDevGetReaderInfo		
    * Description:	获取读写器属性信息
    * Input:       icdev:	通讯设备句柄		
    *              flag:  0x00-是否支持身份证
    * Output:		data: flag 对应的数据	
    * Return:		data 长度,如果返回值小于0表示错误，请查看错误代码表
    * Others: 
    */
    int STDCALL mwDevGetReaderInfo(HANDLE icdev, int flag, char data[]);
    /*
    ...
    */
	...
	/*
	* Function:	mwDevGetCompanyCode
	* Description:	获取终端机唯一编码（3.0.0.21及以后版本）
	* Input:       icdev:	通讯设备句柄
	* Output:		strCompanyCode:   以'\0'为结尾的ANSI字符串。请最少预留17字节的存储空间
	* Return:		字符串长度,如果返回值小于0表示错误，请查看错误代码表
	* Others:
	*/
	int STDCALL mwDevGetCompanyCode(HANDLE icdev, char *strCompanyCode);

	/*
	* Function:	mwDevGetCompanyCode2
	* Description:	获取终端机唯一编码（3.0.0.20及以前版本）
	* Input:       icdev:	通讯设备句柄
	* Output:		strCompanyCode:   以'\0'为结尾的ANSI字符串。请最少预留17字节的存储空间
	* Return:		字符串长度,如果返回值小于0表示错误，请查看错误代码表
	* Others:
	*/
	int STDCALL mwDevGetCompanyCode2(HANDLE icdev, char *strCompanyCode);
    ...
    * Others:    
    */
    //int STDCALL mwSmartCardReset(HANDLE icdev, int slotNumber, unsigned char atrInfo[]);
    ...
    */
	...
	//int STDCALL mwSmartCardReset_HEX(HANDLE icdev, int slotNumber, char *strArtInfo);
    ...
    */
    ...
    /*
    * Function:     mwFelicaCommand		
    * Description:	Felica卡传输指令
    * Input:        icdev: 通讯设备句柄    
    *               slotNumber: 卡座编号
    *                   0	:   接触式卡座
    *                   1	:   非接触式大卡座
    ...
    * Others:     
    */
    int STDCALL mwFelicaCommand(HANDLE icdev, int slotNumber, const unsigned char srcData[], int srcLen, unsigned char dstInfo[]);
    /*
    ...
    *                   0x00    STD模式,只能寻到空闲状态下的卡片，被激活或停活（Halt）的卡片不会响应
    *                   0x01    ALL模式,能寻到空闲状态和已经被停活（Halt）的卡片    
    * Output:       decSnrStr:    卡片序列号(10进制)
    * Return:       卡号字节长度,如果返回值小于0表示错误，请查看错误代码表
    * Others:       该接口最大只能找到卡号为4/7 byte 的卡片
    */
    int STDCALL mwOpenCardDecimalStr(HANDLE icdev, int openMode , char* decSnrStr);
    ...
    * Others:     
    */
    int STDCALL mwMifareLoadKey(HANDLE icdev, int mode, int sectorNo, const unsigned char key[]);
    ...
    * Others:     
    */
    int STDCALL mwMifareAuthLoadKey(HANDLE icdev, int mode, int sectorNo);
    /*
    * Function:	mwCheckCard     
    * Description:	检测存储卡类型
    * Input:       icdev:      通讯设备句柄
    * Output:      NULL
    * Return:		>=0	表示真正的卡片类型;	<0	错误
    * Others:    目前只支持CPU,24C01A,24C02,24C04,24C08,24C16,AT88SC102,AT88SC1604,AT88SC1608,AT88SC153
	*			 测试卡型原理是对卡片进行试操作,读取特征字节或试读写数据,测试类型不一定100%准确。
	*			 如4442卡6、7字节为0x81,0x15,若此两字节被恶意修改,将无法测试出卡片类型。
    */
    int STDCALL mwCheckCard(HANDLE icdev);
	 /*
    * Function:	mwSelectCardType     
    * Description:	选择存储卡类型
    * Input:       icdev:      通讯设备句柄
	* Input:       CardType:   存储卡类型
    * Output:      NULL
    * Return:		>=0	表示正确;	<0	错误   
    */
    int STDCALL mwSelectCardType(HANDLE icdev, int CardType);
    ...
    * Input:		cardType:
    *                 1---  AT24C01A 卡容量: 01 * 1024/8 = 128 字节
    *                 2---  AT24C02 卡容量: 02 * 1024/8 = 256 字节
    *                 3---  AT24C04 卡容量: 04 * 1024/8 = 512 字节
    *                 4---  AT24C08 卡容量: 08 * 1024/8 = 1024 字节
    *                 5---  AT24C16 卡容量: 16 * 1024/8 = 2048 字节
    *                 11---  AT24C32 卡容量: 32 * 1024/8 = 4096 字节
    *                 6---  AT24C64 卡容量: 64 * 1024/8 = 8192 字节
    *                 7---  AT24C128 卡容量: 128 * 1024/8 = 16384 字节
    *                 8---  AT24C256 卡容量: 256 * 1024/8 = 32768 字节
    *               offset: 偏移地址
    ...
    */
	...
	/**
	* Function:	mwRead24CXX_CardType 2017.05.26添加
	* Description:	读24CXX系列卡片，带卡号
	* Input:		cardType:
	*                 1---  AT24C01A 卡容量: 01 * 1024/8 = 128 字节
	*                 2---  AT24C02 卡容量: 02 * 1024/8 = 256 字节
	*                 3---  AT24C04 卡容量: 04 * 1024/8 = 512 字节
	...
	*/
	int STDCALL mwRead24CXX_CardType(HANDLE icdev, int cardType, int offset, int length, unsigned char data[]);

	/**
	* Function:	mwWrite24CXX_CardType  2017.05.26 cdd
	* Description:	写24CXX系列卡片，带卡号
	* Input:		cardType:
	*                1---  AT24C01A 卡容量: 01 * 1024/8 = 128 字节
	*                 2---  AT24C02 卡容量: 02 * 1024/8 = 256 字节
	*                 3---  AT24C04 卡容量: 04 * 1024/8 = 512 字节
	...
	* Others:		AT24CXX卡（XX为01A、02、04、08、16、32、64、128、256）是XXKb位的非加密存储器卡，只存在读、写两种操作。
	*/
	int STDCALL mwWrite24CXX_CardType(HANDLE icdev, int cardType, int offset, int length, const unsigned char data[]);
...
//=======================================工具函数====================================================
    ...
    */
	...
	//======================================NT3H1101/NT3H1201 ===============================
	/**
	* Function:	mwNT3H1101GetLableInfo
	* Description:	获取NT3H1101标签信息
	* Input:		icdev:    打开设备句柄
	*
	* Output:		LableData:	标签信息。
	* Return:		标签信息长度,如果返回值小于0表示错误，请查看错误代码表
	* Others:
	*/
	int STDCALL mwNT3H1101GetLableInfo(HANDLE icdev, unsigned char LableData[]);

	/**
	* Function:	mwNT3H1101SelectSectorNo
	* Description:	NT3H1101标签选择操作的扇区
	* Input:		icdev:    打开设备句柄
	*				sectorNo  扇区号
	*
	* Output:		NULL
	...
	* Others:
	*/
	int STDCALL mwNT3H1101SelectSectorNo(HANDLE icdev, int sectorNo);

	/**
	* Function:	mwNT3H1101ReadPageData
	* Description:	NT3H1101标签读页数据
	* Input:		icdev:    打开设备句柄
	*				page      页
	*
	* Output:		pageData  页数据信息
	* Return:		如果返回值小于0表示错误，请查看错误代码表
	* Others:
	*/
	int STDCALL mwNT3H1101ReadPageData(HANDLE icdev, int page, unsigned char pageData[]);

	/**
	* Function:	mwNT3H1101ReadPagesData
	* Description:	NT3H1101标签读多页数据
	* Input:		icdev:    打开设备句柄
	*				startpage 起始页
	*				endpage   结束页
	*
	* Output:		pagesData  多页数据信息
	* Return:		如果返回值小于0表示错误，请查看错误代码表
	* Others:
	*/
	int STDCALL mwNT3H1101ReadPagesData(HANDLE icdev, int startpage, int endpage, unsigned char pagesData[]);

	/**
	* Function:	mwNT3H1101WritePageData
	* Description:	NT3H1101标签写页数据
	* Input:		icdev:    打开设备句柄
	*				startpage 起始页
	*               pagesData 多页数据信息
	*
	...
	* Others:
	*/
	int STDCALL mwNT3H1101WritePageData(HANDLE icdev, int startpage, const unsigned char pagesData[]);
...
//=======================================密码键盘/LCD/语音/磁条卡，R6/F1系列不支持此处的接口===============================
    ...
    */
	...
	/**
	* Function:	    mwKeyboardInputDecode
	* Description:	查询 ANSI X9.8格式的密文密码解析为明文密码
	* Input:		cpass   加密密码， 使用工作密钥加密异或后返回16进制字符串
	*               cardno  卡号， 参见ANSI X9.8 标准 
	*               workKey 工作密钥， 参见ANSI X9.8 标准  16位       DES解密
	*                                                      32位或48位 3DES解密                
	* Output:		key 输入的密码，返回的经过工作密钥加密后的数据(16进制字符串格式)， 加密前的数据按照ANSIX9.8标准组装
	* Return:		密码位数（根据客户输入而定）key就是密码，<0	错误
	*/
	//int STDCALL mwPassToolGetInput(const char *cpass, const char *cardno, const char *workKey, char* key);
	int STDCALL mwKeyboardInputDecode(const char *cpass, const char *workKey, const char *cardno, char* key);
	...
	*                             0x03-请输入旧密码,同时LCD显示提示信息
	*                             0x04-请输入新密码 ,同时LCD显示提示信息
	*               flag 传输方式，0x00明文传输，0x01密文传输
	*               cardno 卡号， 参见ANSI X9.8 标准, see mwPassGetInput()
	* Output:		无
	* Return:		>=0	正确;	<0	错误
	* Others:
	*/
	int STDCALL mwPassIn2(HANDLE icdev, int ctime, int cmd, int flag, const char *cardno);
    ...
    */
	...
	/**
    * Function:	    mwPasiGetInput
    * Description:	使用缺省的工作密钥，获取ANSI X9.8格式的密文密码（外接键盘）,该函数会一直等待用户输入直到超时	
    * Input:		ctime 等待用户按键输入的超时时间，以second为单位；最大255s，超过该时间退出. 如果为0 则表示使用设备默认超时。            
    *               cmd 语音提示, 
    *                             0x00-不提示, 关闭密码键盘,其他参数可以填空
    *                             0x01-请输入密码,同时LCD显示提示信息
    ...
    * 结果为：  0x06 0x12 0x53 0xDF 0xFE 0xDC 0xBA 0x98
    */
    int STDCALL mwPasiGetInput(HANDLE icdev, int ctime, int cmd, int passwordLen, const char *cardno, char *cpass);
    ...
    */
	...
	/**
	* Function:	mwReadMagCard_Noflag
	* Description:	读取磁条卡数据-无起始符和结束符
	* Input:		ctime 等待用户刷卡超时时间，以second为单位；最大255s，最小1s；超过该时间退出.
	* Output:		pMagCardData 存储磁条卡数据,格式如下
	*               <TRACK1>data</TRACK1><TRACK2>data</TRACK2><TRACK3>data</TRACK3>
	* Return:		>=0	正确;	<0	错误
	* Others:
	*/
	int STDCALL mwReadMagCard_Noflag(HANDLE icdev, int ctime, char *pMagCardData);
    ...
    * Others:		
    */
    int STDCALL mwIDCardRequest();
	
		/**
    * Function:	    mwIDCardSelect
    * Description:	选卡		
    * Input:		无
    * Output:		无
    * Return:		=0	正确;	其它  错误	
    * Others:		
    */
    int STDCALL mwIDCardSelect();

	/**
    * Function:	    mwIDCardRead
    * Description:	读取身份证二代证（支持同一张卡循环读卡）		
    * Input:		无
    * Output:		无
    * Return:		=0	正确;	其它  错误	
    * Others:		
    */
	...
	/**
    * Function:	    mwIDCardPhysicalUid
    * Description:	获得二代证卡片物理卡号（UID）	
    * Input:		无
    * Output:		卡片物理卡号（UID）
    * Return:		卡片物理卡号长度
    * Others:		
    */
	int STDCALL mwIDCardPhysicalUid(char* PhysicalUid);
    ...
    */
	...
	/**
    * Function:	    mwIDCardQueryPassNumber
    * Description:	获得港澳台居住证通行证号码		
    * Input:		无
    * Output:		无
    * Return:		港澳台居住证通行证号码
    * Others:		
    */
    const char* STDCALL mwIDCardQueryPassNumber();

	/**
    * Function:	    mwIDCardQueryPoliceNumber
    * Description:	获得港澳台居住证签发次数	
    * Input:		无
    * Output:		无
    * Return:		港澳台居住证签发次数
    * Others:		
    */
    const char* STDCALL mwIDCardQueryPoliceNumber();

		/**
    * Function:	    mwIDCardQueryCertType
    * Description:	获得证件类别	
    * Input:		无
    * Output:		无
    * Return:		证件类别
    * Others:		
    */
    const char* STDCALL mwIDCardQueryCertType();

	/**
    * Function:	    mwIDCardQueryPic
    * Description:	获得身份证照片	
    * Input:		无
    * Output:		无
    * Return:		身份证照片
    * Others:		
    */
	const unsigned char* STDCALL mwIDCardQueryPic();

	/**
    * Function:	    mwIDCardQueryNationCode
    * Description:	获得民族代码或外国人国籍代码英文	
    * Input:		无
    * Output:		无
    * Return:		民族代码或外国人国籍代码英文
    * Others:		
    */
	const char* STDCALL mwIDCardQueryNationCode();

	/**
    * Function:	    mwIDCardQueryPhotoFile
    * Description:	获得身份证照片文件到指定位置：bmp或jpg		
    ...
    */    
	...
	/**
    * Function:	    mwIDCardQueryFingerprintFile
    * Description:	获得指纹信息	
    * Input:		无
    * Output:		FingerprintFile:需要存放的指纹数据
    * Return:		=0	正确;	其它  错误	
    * Others:		
    */
    int STDCALL mwIDCardQueryFingerprintFile(char* FingerprintFile);

	//读文字信息、照片信息，小于0表示失败（不支持同一张卡循环读卡）
	int STDCALL mwIDCardReadNoSame(); 
	//读文字信息、照片信息、指纹信息，小于0表示失败（支持同一张卡循环读卡）
	int STDCALL mwIDCardReadAll(); 
	//读文字信息、照片信息、指纹信息，小于0表示失败（不支持同一张卡循环读卡）
	int STDCALL mwIDCardReadAllNoSame(); 
	//读取追加地址信息，小于0表示失败
	int STDCALL mwIDCardReadAppendMsg();
	//读SamA模块编号，返回读取到的长度，小于0表示失败
	int STDCALL mwIDCardReadSamA(char* samANumber);
	//获取图片base64编码，需先调用mwIDCardQueryPhotoFile
	void STDCALL iGetPhotobuf(char* Photobuf);
	//获取bmp图片base64编码
	void STDCALL iGetPhotobufbmp(char* Photobuf);
	//获取jpg图片base64编码
	void STDCALL iGetPhotobufjpg(char* Photobuf);
	//取得bmp照片文件位置
	void STDCALL iGetPhotobuffilebmp(char* Photobuffile);
	//取得jpg照片文件位置
	void STDCALL iGetPhotobuffilejpg(char* Photobuffile);
	//获取身份证信息
	int STDCALL iReadIDCard(char* pOutInfo);
...
函数说明：读银行卡卡号
参数说明：icdev: 通讯设备标识符
cBankCard：读取到的卡号，以'\0'为结尾的字符串
返回信息：返回0表示成功。返回其他信息表示在操作过程中有错误发生
*/
int STDCALL SIReaderBankCard(HANDLE icdev, char *cBankCard);

/// <summary>
/// 读取社保卡基本信息(仅支持二代社保卡)
/// </summary>
/// <param name="icdev">设备句柄，参数类型：输入</param>  
/// <param name="cardNo">卡号，参数类型：输出</param>  
/// <param name="idNumber">社会保障号码，参数类型：输出</param>  
/// <param name="name">姓名，参数类型：输出</param>  
/// <param name="sexCode">性别，参数类型：输出</param>  
/// <returns>如果返回值小于0表示错误，请查看错误代码表</returns>
int STDCALL mwReadSSCardInfo(HANDLE icdev, char cardNo[], char idNumber[], char name[], int *sexCode);

/// <summary>
/// 读取社保卡基本信息(第三代社保卡兼容二代社保卡)
/// </summary>
/// <param name="icdev">设备句柄，参数类型：输入</param>  
/// <param name="cardNo">卡号，参数类型：输出</param>  
/// <param name="idNumber">社会保障号码，参数类型：输出</param>  
...
/// <param name="sexCode">性别，参数类型：输出</param>  
/// <returns>如果返回值小于0表示错误，请查看错误代码表</returns>
int STDCALL mwReadSSCardInfo2(HANDLE icdev, char cardNo[], char idNumber[], char name[], int *sexCode);

/// <summary>
/// 读居民健康卡基本信息(非接CPU卡)
/// </summary>
/// <param name="icdev">设备句柄，参数类型：输入</param>  
/// <param name="OutputData">读居民健康卡基本信息(非接CPU卡)，参数类型：输出</param>  
/// <returns>如果返回值小于0表示错误，请查看错误代码表</returns>
int STDCALL mwReadJKCardInfo(HANDLE icdev, char *OutputData);

/// <summary>
/// 打开/关闭二维码引擎
/// </summary>
/// <param name="data">Data[0]:	0：表示关闭扫描头引擎  0x01-0xFE: 打开引擎的时间，时间单位：秒 0xFF: 表示一直打开引擎。</param>  
/// <param name="RecvshowData">获取到的二维码数据。</param>
/// <returns>二维码数据长度,如果返回值小于0表示错误，请查看错误代码表</returns>
int STDCALL mwQRCodeOprate(int data, unsigned char RecvshowData[]);

/// <summary>
/// 发送要显示的二维码的内容
/// </summary>
/// <param name="showTime">Data[0]:	0：表示关闭二维码显示，恢复正常显示  0x01-0xFE: 显示时间，单位：秒 0xFF: 表示一直显示二维码，直到接到关闭二维码指令。</param>  
/// <param name="showDatalen">Data[1]-Data[2]: 表示显示内容的长度</param>
/// <param name="showData">Data[3]-Data[n]：要显示的二维码内容：以字符串的形式发送：数字字母符号等以ASCII方式，汉字以GB2312内码方式。</param>
/// <returns>00 表示操作成功，非0表示操作失败</returns>
int STDCALL mwQRCodeShow(int showTime,int showDatalen,const unsigned char showData[]);

/// <summary>
/// 交易结束指令
/// </summary>    
/// <returns>00 表示操作成功，非0表示操作失败</returns>
int STDCALL mwQRCodeEndTrans();

/// <summary>
/// 发送交易信息
/// </summary>
/// <param name="result">Data[0]: 0：表示交易成功，后面数据为交易信息，有效 非0： 表示交易失败</param>
/// <param name="transDatalen">Data[1]: 交易信息的长度</param>
/// <param name="transData">Data[2]- Data[8]：固定长度7个字节：交易信息：金额 0000.00-9999.99之间，
/// 以ASCII方式表示， 大端方式表示，比如成交金额为38.91元：则Data[1]- Data[7]：0x30 0x30 0x33 0x38 0x20 0x39 0x31，
/// 小数点后固定2位  Data[9]- Data[n]: 交易项目名称：以字符串方式表示，字母和数字符合ASCII,汉字符合GB2312内码
/// </param>
/// <returns>00 表示操作成功，非0表示操作失败</returns>
int STDCALL mwQRCodeSendTrans(int result, int transDatalen, const unsigned char transData[]);

/// <summary>
/// 发送交易的语音提示功能
/// </summary>  
/// <param name="data">Data[0]:	0：不发声  1: 支付成功 2：支付不成功
///        3：请扫描二维码 4：请出示付款码 5：扫码成功 6：等待超时
///        7：扫码失败，请正确提供付款码 8：请在手机上输入支付密码。</param>
/// <returns>00 表示操作成功，非0表示操作失败</returns>
int STDCALL mwQRCodeSendVoctips(int data);

/// <summary>
/// 打开/关闭交易的语音提示功能
/// </summary>  
/// <param name="data">Data[0]:	0：关闭  1: 打开。</param>  
/// <returns>00 表示操作成功，非0表示操作失败</returns>
int STDCALL mwQRCodeSetVoctips(int data);
...
/// <param name="data">Data[0]:	0：关闭  1: 打开。</param> 
/// <returns>00 表示操作成功，非0表示操作失败</returns>
int STDCALL mwQRCodeSetLed(int data);

/// <summary>
/// 二维码扫描头工作模式切换：点动模式和连续自动扫描
/// </summary>    
/// <param name="data">Data[0]:	0：点读模式  1: 连续自动扫描。</param> 
/// <returns>00 表示操作成功，非0表示操作失败</returns>
int STDCALL mwQRCodeSetDMable(int data);
...
/// <param name="data">Data[0]:	0：关闭  1: 打开。</param> 
/// <returns>00 表示操作成功，非0表示操作失败</returns>
int STDCALL mwQRCodeSetBarCode(int data);
...
/// <param name="data">Data[0]:	0：关闭  1: 打开。</param> 
/// <returns>00 表示操作成功，非0表示操作失败</returns>
int STDCALL mwQRCodeSetQRable(int data);


//====================电子健康卡接口===============================
int STDCALL YHD_OpenDev(int port,int baud,char *ErrMsg);
int STDCALL YHD_CloseDev(int handle,char *ErrMsg);
int STDCALL YHD_ReadEF05(int handle,char *Info,char *ErrMsg);
int STDCALL YHD_ReadEF06(int handle,char *Info,char *ErrMsg);
int STDCALL YHD_ReadIDCardInfo(int handle,char *Info,char *bmpPath,char *ErrMsg);
int STDCALL YHD_ReadM1(int handle,char *Info,char *ErrMsg);
int STDCALL mwOpenDev(int port,int baud,char *ErrMsg);
int STDCALL mwCloseDev(int handle,char *ErrMsg);
int STDCALL mwReadEF05(int handle,char *Info,char *ErrMsg);
int STDCALL mwReadEF06(int handle,char *Info,char *ErrMsg);
int STDCALL mwReadIDCardInfo(int handle,char *Info,char *bmpPath,char *ErrMsg);
int STDCALL mwReadM1(int handle,char *Info,char *ErrMsg);

//====================全国电子健康卡交叉一键读卡应用接口===============================
int STDCALL mwReadID_SSCARD(char *Info,char *ErrMsg);
