...
#define		UpcasecharA(ch)	( ch >= 'A'?ch:(ch - 'a' + 'A'))
    ...
    */
    ...
    int Evolis_Z390_Printer::Print_Open(char* pPort, char* pPortParam, char* pszRcCode)
    {
        ...
        if (pEvolisPriner)
            return pEvolisPriner->On_Print_Open(pPort,pPortParam,pszRcCode);
        else
        {
            strcpy(pszRcCode,"0001");
            return 1;
    ...
    int Evolis_Z390_Printer::Print_Close(char* pszRcCode)
    {
        ...
        if (pEvolisPriner)
            return pEvolisPriner->On_Print_Close(pszRcCode);
        else
        {
            strcpy(pszRcCode,"0001");
            return 1;
    ...
    int Evolis_Z390_Printer::Print_EnableCard(long lTimeout, int nCheckMode, char* pszRcCode)
    {
        Funclog();
        if (pEvolisPriner)
            return pEvolisPriner->On_Print_EnableCard2(lTimeout,nCheckMode,pszRcCode);
        else
        {
            strcpy(pszRcCode,"0001");
            return 1;
    ...
    int Evolis_Z390_Printer::Print_DisableCard(long lTimeout, char* pszRcCode)
    {
        Funclog();
        if (pEvolisPriner)
            return pEvolisPriner->On_Print_DisableCard(lTimeout,pszRcCode);
        else
        {
            strcpy(pszRcCode,"0001");
            return 1;
    ...
    int Evolis_Z390_Printer::Print_Eject(long lTimeout, char* pszRcCode)
    {
        Funclog();
        if (pEvolisPriner)
            return pEvolisPriner->On_Print_Eject(lTimeout,pszRcCode);
        else
        {
            strcpy(pszRcCode,"0001");
            return 1;
    ...
    int Evolis_Z390_Printer::Print_Retract(long lTimeout, int nBoxNo, char* pszRcCode)
    {
        Funclog();
        if (pEvolisPriner)
            return pEvolisPriner->On_Print_Retract(lTimeout,nBoxNo,pszRcCode);
        else
        {
            strcpy(pszRcCode,"0001");
            return 1;
    ...
    int Evolis_Z390_Printer::Print_Dispense(long lTimeout, int nBox, int nDispPos, char* pszRcCode)
	{
        Funclog();
        if (pEvolisPriner)
            return pEvolisPriner->On_Print_Dispense(lTimeout,nBox,nDispPos,pszRcCode);
        else
        {
            strcpy(pszRcCode,"0001");
            return 1;
    ...
    int Evolis_Z390_Printer::Print_GetBoxStatus(long lTimeout, Lithograph::LPLITHOGRAPHBOXINFO& lpBoxInfo, char* pszRcCode)
    {
        Funclog();
        if (pEvolisPriner)
            return pEvolisPriner->On_Print_GetBoxStatus(lTimeout, lpBoxInfo,  pszRcCode);
        else
        {
            strcpy(pszRcCode,"0001");
            return 1;
    ...
    int Evolis_Z390_Printer::Print_Status(long lTimeout, Lithograph::LPLITHOGRAPHSTATUS& lpStatus, char* pszRcCode)
    {
        Funclog();
        if (pEvolisPriner)
            return pEvolisPriner->On_Print_Status(lTimeout, lpStatus,  pszRcCode);
        else
        {
            strcpy(pszRcCode,"0001");
            return 1;
    ...
    int Evolis_Z390_Printer::Print_InitPrint(long lTimeout, char* pszRcCode)
    {
        Funclog();
        if (pEvolisPriner)
            return pEvolisPriner->On_Print_InitPrint(lTimeout, pszRcCode);
        else
        {
            strcpy(pszRcCode,"0001");
            return 1;
    ...
    int Evolis_Z390_Printer::Print_PrintImage(long lTimeout, char* pImage, int nAngle, float fxPos, float fyPos, float fHeight, float fWidth, char* pszRcCode)
    {
        ...
        if (!pImage || !strlen(pImage))
        {
            strcpy(pszRcCode, "0001");
            RunlogF("The path of the picture is empty or invalid.\n");
            return 1;
        }
        PicInfoPtr pPicInfo = make_shared<PicInfo>();
        pPicInfo->picpath = pImage;
        pPicInfo->nAngle = nAngle;
        pPicInfo->fxPos = fxPos;
        pPicInfo->fyPos = fyPos;
        pPicInfo->fHeight = fHeight;
        pPicInfo->fWidth = fWidth;
        pPicInfo->nType = 1;
        auto itFind = find_if(pEvolisPriner->m_picInfo.begin(),pEvolisPriner->m_picInfo.end(),[&](PicInfoPtr &pItem)
        {
            return *pItem == *pPicInfo;
        });
        if (itFind != pEvolisPriner->m_picInfo.end())
        {
            strcpy(pszRcCode, "0000");
            RunlogF("Input a same picture at the same position.\n");
            return 0;
        }

        pEvolisPriner->m_picInfo.push_back(pPicInfo);
   ...
   bool Evolis_Z390_Printer::RunApdu(DEVHANDLE hReader, string cmd,string &OutMsg)
    ...
    int Evolis_Z390_Printer::Print_StartPrint(long lTimeout, char* pszRcCode)
    {
        Funclog();
        if (pEvolisPriner)
            return pEvolisPriner->On_Print_StartPrint(lTimeout,  pszRcCode);
        else
        {
            strcpy(pszRcCode,"0001");
            return 1;
