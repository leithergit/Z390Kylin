...
// int Print_ExtraCommand(long lTimeout, char* pCommand, LPVOID lpCmdIn, LPVOID& lpCmdOut, char* pszRcCode)
#define evolis_command(printer, cmd,  cmdSize,reply, replyMaxSize)  printer->Print_ExtraCommand(5000,(char *)cmd,nullptr,(LPVOID &)reply,pszRcCode)
...
#define     CheckResult(x)  if (!x){\
                                ...
                                RunlogF(msg.c_str());\
                                return 1;}
#define LibVer "Z390_V1.0.0.1"

struct PrinterLibInfo
{
    std::string strPrinter;
    std::string strLib;
};

list<PrinterLibInfo> Printerlist =
{
...
//        }
    ...
    {
        Funclog()
        if (!pPort  ||!pPortParam|| !strlen(pPort) || !strlen(pPortParam))
        {
            strcpy(pszRcCode, "0001");
            RunlogF("Printer name is invalid.\n");
            return 1;
        }
        if (m_hReader)
            dc_exit(m_hReader);
        m_hReader = dc_init(100,115200);
        if (!m_hReader)
        {
            strcpy(pszRcCode, "0002");  // 打开读卡器失败
            RunlogF("Failed in open card Reader.\n");
            return 1;
        }
        if (!pEvolisPriner)
        {
            ...
            QString strLibPath = strCurrentPath + "/" + strPrinterLib.c_str();
            ...
            if (pLibhandle)
            {
                ...
                {
                    strcpy(szError,"Failed in loading procedure CreateInstance!");
                    qDebug()<<szError;
                    if (pLibhandle)
                    {
                        dlclose(pLibhandle);
                    ...
                    strcpy(pszRcCode, "0012");
                    return 12;
                }
                pEvolisPriner = (CLithographPrinter*)pCreateInst(nullptr);
                if (!pEvolisPriner)
                {
        ...
        const char* path = "./printer.log";
        LPVOID reply;
        char   ec[5];
        //"USB:/dev/usb/lp0"
        //"IP:192.168.1.1"
        pEvolisPriner->Print_ExtraCommand(0, (char*) "setLogPath", (char*) path, reply, ec);
    ...
    int Evolis_Z390_LithographPrinter::Print_GetVersion(long lTimeout, char* pVersionInfo, char* pszRcCode)
    {
        ...
        sprintf(pVersionInfo,"Evolis_%s_%02d%02d%02d",LibVer,ptminfo->tm_year + 1900, ptminfo->tm_mon + 1, ptminfo->tm_mday);
        ...
        if (pEvolisPriner)
        {
            ...
            */
            ...
            for (auto var : szCommand)
            {
                ...
                Res = pEvolisPriner->Print_ExtraCommand(lTimeout,"sendCommand",(LPVOID)var, reply, ec);
                //Res = evolis_command(pEvolisPriner, var,strlen(var), RespValue,1024);
//                if (Res){
...
//                }
            ...
            if (nCheckMode)
            {
                ...
                while (!bCardReady)
                {
                    ...
                    nResult = Print_Status(lTimeout,lpStatus,pszRcCode);
                    if (nResult == 0 )
                    {
                ...
                Print_DisableCard(lTimeout,pszRcCode);
            }
...
//            int nIdx = 0;
//            RunlogF("Try to evolis_command(%s).\n",szCmd[nIdx]);
//            int Res = pEvolisPriner->Print_ExtraCommand(lTimeout,"sendCommand",(LPVOID)szCmd[nIdx], reply, szReply);
...
//            nIdx ++;
//            RunlogF("Try to evolis_command(%s).\n",szCmd[nIdx]);
//            Res = pEvolisPriner->Print_ExtraCommand(lTimeout,"sendCommand",(LPVOID)szCmd[nIdx], reply, szReply2);
...
//                LPVOID reply = NULL;
//                RunlogF("Try to evolis_command(%s).\n",var);
//                Res = pEvolisPriner->Print_ExtraCommand(lTimeout,"sendCommand",(LPVOID)var, reply, szReply);
...
////            char ec[5];
////            int  r;
////            r = pEvolisPriner->Print_ExtraCommand(lTimeout, (char*) "setOutputTray",(char*) "STD_STANDBY", reply, ec);
////            r = pEvolisPriner->Print_ExtraCommand(lTimeout, (char*) "setCardSideEject",(char*) "FRONT_SIDE_UP", reply, ec);
////            r = pEvolisPriner->Print_Eject(lTimeout, ec);
        }
        else
        {
            strcpy(pszRcCode,"0001");
        ...
        if (m_hReader)
        {
		 ...
		 // 设置用户卡座
			...
			if (dc_cpureset_hex(m_hReader, (unsigned char*)&ret, dataBuffer))
			{
                ...
                RunlogF("Failed in dc_cpureset_hex.\n");
				return 1;
			}
...
//                 char   ec[5];
//                 RunlogF("Try to evolis_command(%s).\n",var);
//                 r = pEvolisPriner->Print_ExtraCommand(lTimeout,"sendCommand",(LPVOID)var, reply, ec);
//                 RunlogF("Result of evolis_command(%s):%d.\n",var,r);
//                 //if (evolis_command(pEvolisPriner,var,strlen(var),szReply,sizeof(szReply)) != 0)
////                 {
////                     RunlogF("Failed in evolis_command(%s).\n",var);
////                     if (reply)
////                        strcpy(pszRcCode, (char *)reply);
////                     else
////                        strcpy(pszRcCode, "0001");
////                     return 1;
////                 }
...
//            strcpy(pszRcCode, "0000");
    ...
    int Evolis_Z390_LithographPrinter::Print_PrintImage(long lTimeout, char* pImage, int nAngle, float fxPos, float fyPos, float fHeight, float fWidth, char* pszRcCode)
    {
        Funclog();
        if (pEvolisPriner)
            return pEvolisPriner->Print_PrintImage(lTimeout, pImage, nAngle, fxPos,fyPos, fHeight,fWidth,pszRcCode);
        else
        {
            strcpy(pszRcCode,"0001");
            return 1;
    ...
    int Evolis_Z390_LithographPrinter::Print_PrintText(long lTimeout, char* pText, int nAngle, float fxPos, float fyPos, char* pFontName, int nFontSize, int nFontStyle, int nColor, char* pszRcCode)
    {
        ...
        Funclog();
        if (pEvolisPriner)
            return pEvolisPriner->Print_PrintText(lTimeout, pText, nAngle, fxPos, fyPos, pFontName, nFontSize, nFontStyle, nColor, pszRcCode);
        else
        {
            strcpy(pszRcCode,"0001");
            return 1;
    ...
    int Evolis_Z390_LithographPrinter::Print_PrintBarcode(long lTimeout, int nAngle, char* pBarcodeType, float fxPos, float fyPos, int nMultiplier, int nHeight, int nIsActive, char* pBarcodeText, char* pszRcCode)
    {
        ...
        Funclog();
        if (pEvolisPriner)
            return pEvolisPriner->Print_PrintBarcode(lTimeout, nAngle, pBarcodeType, fxPos, fyPos, nMultiplier, nHeight, nIsActive, pBarcodeText, pszRcCode);
        else
        {
            strcpy(pszRcCode,"0001");
            return 1;
   ...
   bool Evolis_Z390_LithographPrinter::RunApdu(HANDLE hReader, string cmd,string &OutMsg)
	{
        Funclog();
    ...
    int Evolis_Z390_LithographPrinter::Print_Rotate(long lTimeout, int nAngle, char* pszRcCode)
    {
        Funclog();
        if (pEvolisPriner)
            return pEvolisPriner->Print_Rotate(lTimeout, nAngle, pszRcCode);
        else
        {
            strcpy(pszRcCode,"0001");
            return 1;
    ...
    int Evolis_Z390_LithographPrinter::Print_CloseSleepMode(long lTimeout, char* pszRcCode)
    {
        Funclog();
        if (pEvolisPriner)
            return pEvolisPriner->Print_CloseSleepMode(lTimeout,  pszRcCode);
        else
        {
            strcpy(pszRcCode,"0001");
            return 1;
    ...
    int Evolis_Z390_LithographPrinter::Print_ExtraCommand(long lTimeout, char* pCommand, LPVOID lpCmdIn, LPVOID& lpCmdOut, char* pszRcCode)
    {
        ...
        if (!pEvolisPriner)
        {
            if (strcmp(pCommand,"Set Printer Model") ||!lpCmdIn )
            {
                strcpy(pszRcCode,"0001");
                return 1;
            }

            string strPrinter = (const char *)lpCmdIn;
            ...
            strPrinterLib = itFind->strLib;
            strcpy(pszRcCode,"0000");
            return 0;
        }

        if (0 == strcmp(pCommand, "ReadBankNo")) //读取银行卡号
		{
		...
		else
		{
            ...
            {
                return pEvolisPriner->Print_ExtraCommand(lTimeout, pCommand, lpCmdIn, lpCmdOut, pszRcCode);
            }
		}
        return 0;
    }

    bool Evolis_Z390_LithographPrinter::WriteFile(HANDLE dcHandle, string fileID, string tag, string val,string &msg)
	{
        Funclog();
		char valHex[32] = { 0 };
    ...
    bool Evolis_Z390_LithographPrinter::WriteCA(HANDLE dcHandle, string fileID, string tag, string val, string &msg)
	{
		char valHex[32] = { 0 };
		sprintf(valHex, "%02X", val.length() / 2);
		string tlv = tag + valHex + val;
